...
#				self._col_picked_lab.pixmap().fill(QColor(*col[:3]) )
	...
	def mouseMoveEvent (self, event) :
		if self._mouse_ini_pos is not None :
			old_rect = QtCore.QRect(self._select_rect.topLeft(),
			                 self._select_rect.size() + QtCore.QSize(1,1) )
			self._select_rect = self.selected_rect(event.pos() )
			update_rect = QtCore.QRect(self._select_rect.topLeft(),
			                 self._select_rect.size() + QtCore.QSize(1,1) ) | old_rect
	...
	def paintEvent (self, event) :
		...
		if self._select_rect is not None :
			...
			pen = QtCore.QPen(QtCore.QColor(255,255,255) )
			pen.setDashPattern([5,5])
			painter.setPen(pen)
			painter.drawRect(self._select_rect)
			pen.setColor(QtCore.QColor(0,0,0) )
			pen.setDashOffset(5.)
			painter.setPen(pen)
			painter.drawRect(self._select_rect)
			painter.end()
