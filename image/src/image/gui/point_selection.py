...
#X         self.version = 2
        ...
        QtCore.QObject.connect(self._load_points,
                               QtCore.SIGNAL("triggered(bool)"),
                               self.load_points)

        # Save points
        self._save_points = self.menu_file.addAction('Save Points')
        QtCore.QObject.connect(self._save_points,
                               QtCore.SIGNAL("triggered(bool)"),
                               self.save_points)
        ...
        QtCore.QObject.connect(self._action_left,
                               QtCore.SIGNAL("triggered(bool)"),
                               self.rotate_left)
        ...
        QtCore.QObject.connect(self._action_right,
                               QtCore.SIGNAL("triggered(bool)"),
                               self.rotate_right)
...
#X         f.close()
    ...
    def add_point (self, pos, my_pid = None) :
        ...
        :Parameters:
         - `pos` (QtCore.QPointF) - position of the point on the screen
        """
        ...
        if img is not None :
            ...
            self.update_pix()
            self.emit(QtCore.SIGNAL("points_changed"),self)
            return pid,(i,j,k)
...
#X                 else:
                ...
                col = QtGui.QColor.fromHsv( (pid * 10) % 360,255,255)
                item = self._scene.addEllipse(QtCore.QRectF(-10,-10,20,20),QtGui.QPen(col),QtGui.QBrush(col)  )
                item.setZValue(10)
